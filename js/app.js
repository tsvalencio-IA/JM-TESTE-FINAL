(function () {
  ...
  const LOGIN_FLOW_VERSION = "jm-v32-7-3-login-sem-travamento";
...
Rota: ${url}`;
  ...
  function viewCallProofs(id) {
    ...
    `;
    const damageParts = Array.isArray(damage.parts) ? damage.parts.map((part) => {
      const detail = [part.type, part.severity, part.note].filter(Boolean).join(" · ");
      return (part.label || part.key) + (detail ? " — " + detail : "");
    }).filter(Boolean).join(" | ") : "";
    const damageHtml = `<h2>Avarias marcadas</h2><p><b>Tipo de veículo:</b> ${esc(damage.vehicleType || "-")}<br><b>Registros:</b> ${esc(damageParts || "-")}<br><b>Descrição geral:</b><br>${esc(damage.details || "-")}</p>`;
  ...
  function aiReviewedCallPayload(index, draft) {
    ...
    reviewed.normalizedCall = {
      ...
      parserVersion: "jm-v32-7-3-login-sem-travamento"
    };
  ...
  async function createOfficialCallFromAi(index) {
    ...
    const callData = compactForFirestore({
      ...
      aiReviewed: true,
      aiCreatedAt: now,
      aiParserVersion: "jm-v32-7-3-login-sem-travamento",
